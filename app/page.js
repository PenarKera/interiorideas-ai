...
export default function Home() {
  ...
  if (!user) return <div style={{ minHeight: "100vh", background: "#0A0A0F", display: "flex", alignItems: "center", justifyContent: "center", color: "#E0D9FF" }}>Duke u ngarkuar...</div>;
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "32px 20px" }}>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ marginTop: 16 }}>
            <label style={{ fontSize: 12, color: "#6B6B8A", display: "block", marginBottom: 6 }}>Kërkesa shtesë</label>
        ...
        {loading && <div style={{ background: "rgba(123,79,255,0.08)", border: "1px solid rgba(123,79,255,0.25)", borderRadius: 12, padding: "20px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16 }}><div style={{ width: 32, height: 32, border: "3px solid rgba(123,79,255,0.2)", borderTopColor: "#7B4FFF", borderRadius: "50%", animation: "spin 0.8s linear infinite", flexShrink: 0 }} /><div><div style={{ fontWeight: 700, fontSize: 15 }}>AI po punon...</div></div></div>}
        {error && <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}><div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>⚠️ Gabim</div><div style={{ fontSize: 13, color: "#FCA5A5" }}>{error}</div></div>}
        {result && !loading && (
          <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(123,79,255,0.2)", borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ marginBottom: 20 }}><div style={{ fontSize: 10, letterSpacing: "0.2em", color: "#7B4FFF", fontWeight: 700, marginBottom: 10 }}>ELEMENTET KYÇE</div><div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{result.key_elements?.map((el,i) => <span key={i} style={{ background: "rgba(123,79,255,0.15)", border: "1px solid rgba(123,79,255,0.3)", borderRadius: 20, padding: "5px 12px", fontSize: 12, color: "#C084FC" }}>{el}</span>)}</div></div>
            <div style={{ marginBottom: 20 }}><div style={{ fontSize: 10, letterSpacing: "0.2em", color: "#7B4FFF", fontWeight: 700, marginBottom: 10 }}>MOBILJE</div>{result.furniture?.map((f,i) => <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, padding: "12px 16px", marginBottom: 8, display: "flex", justifyContent: "space-between" }}><div><div style={{ fontWeight: 700, fontSize: 14 }}>{f.item}</div><div style={{ fontSize: 12, color: "#6B6B8A" }}>{f.description}</div></div><span style={{ color: "#C084FC", fontWeight: 700 }}>{f.approx_price}</span></div>)}</div>
