    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui", padding: 20 }}>
      <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(120,80,255,0.3)", borderRadius: 20, padding: "48px 40px", width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 13, color: "#6B6B8A" }}>{isSignup ? "Create your account" : "Welcome back"}</div>
        </div>
        {isSignup && (
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, color: "#8B8BAA", display: "block", marginBottom: 8 }}>FULL NAME</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(120,80,255,0.3)", borderRadius: 10, padding: "12px 16px", color: "#E0D9FF", fontSize: 14, boxSizing: "border-box" }} placeholder="Your full name" />
          </div>
        )}
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 12, color: "#8B8BAA", display: "block", marginBottom: 8 }}>EMAIL</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(120,80,255,0.3)", borderRadius: 10, padding: "12px 16px", color: "#E0D9FF", fontSize: 14, boxSizing: "border-box" }} placeholder="your@email.com" />
        </div>
        <div style={{ marginBottom: 24 }}>
          <label style={{ fontSize: 12, color: "#8B8BAA", display: "block", marginBottom: 8 }}>PASSWORD</label>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(120,80,255,0.3)", borderRadius: 10, padding: "12px 16px", color: "#E0D9FF", fontSize: 14, boxSizing: "border-box" }} placeholder="Min. 6 characters" />
        </div>
